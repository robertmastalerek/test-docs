...
You can leave the `META-INF/processes.xml` file empty. In that case, default values are used. See the [Empty Processes.xml](/user-guide/latest/user-guide/process-applications/the-processes-xml-deployment-descriptor/#empty-processes-xml) section of the [User Guide](/user-guide/latest/user-guide) for more information
{{< /note >}}
